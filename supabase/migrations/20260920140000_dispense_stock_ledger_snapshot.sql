-- The stock ledger records where a dispense started and ended.
--
-- 20260825110000 added source_type/source_id/quantity_before/quantity_after to
-- stock_movements and populated them from bulk import stock-in and IP request
-- supply -- but it never redefined dispense_prescription, so the busiest
-- writer of all has been leaving those four columns null since the columns
-- existed. The running quantities were never wrong (quantity_delta is, and
-- always was, exact), but a dispense movement could not be tied back to its
-- sale, and the ledger could not be replayed to prove a batch's quantity: the
-- one thing an immutable stock ledger is for.
--
-- Only that insert changes. The rest of the function is reproduced verbatim
-- from the deployed definition so nothing else shifts with it.
begin;

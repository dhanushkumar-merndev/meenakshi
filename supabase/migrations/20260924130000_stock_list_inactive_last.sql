-- Stock screen: deactivated batches list after every active one instead of
-- interleaving by expiry date. b.id is added as a final tiebreak so offset
-- pagination is stable when two batches share medicine batch numbers.
-- Signature is unchanged; only the ORDER BY differs from 20260816270000.
begin;

create index if not exists medicine_batches_list_order_idx
  on public.medicine_batches (active desc, expiry_date, batch_number, id);
...
begin
  ...
  -- Deactivated batches sink to the end so the live shelf stays on page one.
  order by b.active desc, b.expiry_date, b.batch_number, b.id

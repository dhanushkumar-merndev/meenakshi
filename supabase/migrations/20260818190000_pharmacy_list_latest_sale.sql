-- The Completed/All tabs on /pharmacy could reprint the prescription but had
-- no way back to the payment receipt -- that link only ever existed in the
-- DispenseDialog's transient post-dispense state, gone the moment the page
-- is left or reloaded. Surfaces the most recent pharmacy_sales row for each
-- prescription so the page itself can offer a Receipt button. A prescription
-- dispensed across more than one partial transaction has more than one sale;
-- this deliberately shows only the latest -- a full sales history per
-- prescription is a bigger feature than what was asked for here.
begin;

-- Adding an OUT column widens the result row type, which CREATE OR REPLACE
-- cannot do (Postgres: "cannot change return type of existing function").

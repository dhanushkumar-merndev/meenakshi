-- Dispensing a prescription flips it to 'dispensed' and it vanishes from the
-- Pending tab immediately, so printing the receipt/prescription for the
-- patient who is still standing at the counter means switching to Completed
-- first. Keeps a just-dispensed row on the Pending tab for a 10-minute grace
-- window so the print buttons stay one click away while the patient is
-- actually there; it naturally drops off after that.
begin;
...
begin
  ...
  select
    ...
    (select s.id from public.pharmacy_sales s where s.prescription_id = p.id order by s.created_at desc limit 1)
  ...
  where (
    case p_status_filter
      when 'completed' then p.status = 'dispensed'
      when 'all' then p.status in ('pending','partially_dispensed','dispensed','expired','cancelled')
      else p.status in ('pending','partially_dispensed')
        -- Grace window: a prescription dispensed in the last 10 minutes still
        -- shows here too, so the receipt/prescription print buttons stay
        -- reachable without switching tabs while the patient is still there.
        or (p.status = 'dispensed' and p.updated_at > now() - interval '10 minutes')
    end
  )
    and (
      v_query is null

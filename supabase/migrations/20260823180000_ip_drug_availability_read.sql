-- IP staff need the same read-only medicine availability view used by OP
-- staff while coordinating inpatient care. This function deliberately
-- returns no prices, batch details, supplier data, or mutation capability.
begin;
...
begin
 if public.current_app_role() not in ('admin','pharmacy','doctor','op','ip') then raise exception 'forbidden' using errcode='42501';end if;

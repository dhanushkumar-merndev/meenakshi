...
import { FileCheck2,LoaderCircle,NotebookPen,Stethoscope } from "lucide-react";
import { addProgressNote,completeDischarge,saveDischargeSummary } from "./actions";
...
export function ProgressNoteDialog({ticketId,defaultFee}:{ticketId:string;
  /** Doctor's configured IP visit fee in rupees; a suggestion, always editable. */
  defaultFee?:string}){
  const[state,action,pending]=useActionState(addProgressNote,initial);

...
export default async function IpWorkspacePage({
  searchParams,
  profile,
  fixedStatus,
}: {
  searchParams: Promise<IpSearchParams>;
  profile: Profile;
  fixedStatus?: IpTicketScope;
}) {
  const params = await searchParams;
  const requestedStatus = fixedStatus ?? params.status ?? "active";
  const selectedStatus = requestedStatus === "mine" ? "active" : requestedStatus;
  ...
  const mineOnly = requestedStatus === "mine"; const page = Math.max(1, Number(params.page) || 1); const size = 50; const q = params.q?.trim() ?? "";
  ...
  const [ticketsResult, doctorsResult, roomsResult, occupancyResult, referralResult, ipStaffResult] = await Promise.all([
    ...
    // The New Admission dialog is available from every IP list page, so room
    // availability must always come from all live tickets—not from whichever
    // discharged/pending subset happens to be displayed.
    supabase
      .from("ip_tickets")
  ...
  const occupied = new Set(
    ((occupancyResult.data ?? []) as unknown as Occupancy[]).map(
      (ticket) => ticket.room_bed_id,
    ),
  );
  ...
  return (
    <div>
      <PageHeader
        title={view === "grid" ? "Room Occupancy" : requestedStatus === "mine" ? "My IP Patients" : selectedStatus === "all" ? "IP Tickets" : selectedStatus === "discharge_pending" ? "Pending Discharges" : selectedStatus === "discharged" ? "Discharged Patients" : "Current IP Patients"}
        ...
        actions={
          <>
            <FilterTabs
              ...
              active={view}
              param="view"
              params={{ q, status: fixedStatus ? undefined : requestedStatus }}
              tabs={[
      ...
      <Card>
        <CardContent className="p-0">
              ...
              <TableBody>
                {tickets.length ? (
                  ...
                  <TableRow>
                    ...
                    >
                      {q
                        ? "No IP tickets match this search."
                        : requestedStatus === "mine"
                          ? "No current IP patients are assigned to you."
                          : selectedStatus === "discharge_pending"
                            ? "No patients are pending discharge."
                            : selectedStatus === "discharged"
                              ? "No discharged IP tickets."
                              : selectedStatus === "all"
                                ? "No IP tickets found."
          ...
          </div><TablePager page={page} pages={Math.max(1, Math.ceil((ticketsResult.count ?? 0) / size))} total={ticketsResult.count ?? 0} params={{ status: fixedStatus ? undefined : requestedStatus, q, view }} />
        </CardContent>
      </Card>
      </>
      )}
...
export async function IpRoleWorkspacePage({
  searchParams,
  fixedStatus,
}: {
  searchParams: Promise<IpSearchParams>;

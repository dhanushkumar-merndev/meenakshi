...
async function collectExport(month: string, includeDocuments: boolean) {
  ...
  const [visitsR, salesR, ipChargesR, ipPaymentsR, counterCollectionsR, admissionsR, reportsR, auditR, batchesR, settingsR, discountsR] = await Promise.all([
    admin.from("visits").select("id,patient_id,doctor_id,department_id,visit_type,visit_date,token_number,fee_paise,discount_paise,status,related_previous_visit_id,notes,created_by,created_at,updated_at").gte("visit_date", start).lt("visit_date", endDay),
    admin.from("pharmacy_sales").select("id,prescription_id,patient_id,source,ip_ticket_id,total_paise,discount_paise,payment_mode,dispensed_by,created_at").gte("created_at", from).lt("created_at", to),
    ...
    admin.from("ip_inventory_requests").select("id,ip_ticket_id,settlement,counter_collected_paise,counter_discount_paise,counter_payment_mode,counter_reference,counter_collected_at,counter_collected_by,fulfilled_at,fulfilled_by,created_at").eq("settlement", "pharmacy_counter").gte("counter_collected_at", from).lt("counter_collected_at", to),
    ...
    admin.from("hospital_settings").select("hospital_name,address,phone,email,prescription_footer,token_footer,digital_prescription_text,max_discount_percent,updated_at"),
    // Every discount given in the month, voided ones included (with their
    // void details) so the ledger can be reconciled from the export alone.
    admin.from("discounts").select("id,source,visit_id,pharmacy_sale_id,ip_ticket_id,ip_inventory_request_id,procedure_sale_id,patient_id,gross_paise,amount_paise,reason,note,given_by,created_at,voided_at,voided_by,void_reason").gte("created_at", from).lt("created_at", to),
  ]);
  for (const result of [visitsR, salesR, ipChargesR, ipPaymentsR, counterCollectionsR, admissionsR, reportsR, auditR, batchesR, settingsR, discountsR]) if (result.error) throw result.error;
  const discounts = (discountsR.data ?? []) as Row[];
  const visits = (visitsR.data ?? []) as Row[]; const sales = (salesR.data ?? []) as Row[]; const ipCharges = (ipChargesR.data ?? []) as Row[]; const ipPayments = (ipPaymentsR.data ?? []) as Row[]; const counterCollections = (counterCollectionsR.data ?? []) as Row[]; const reports = (reportsR.data ?? []) as Row[];
  ...
  const counterCollectionItems = await inRows(admin, "ip_inventory_request_items", "id,request_id,requested_name,requested_quantity,fulfilled_quantity,unit_price_paise,amount_paise,inventory_item_id,medicine_id,status,created_at", "request_id", ids(counterCollections, "id"));
  const ticketIds = [...new Set([...ids((admissionsR.data ?? []) as Row[], "id"), ...ids(ipCharges, "ip_ticket_id"), ...ids(ipPayments, "ip_ticket_id"), ...ids(counterCollections, "ip_ticket_id"), ...ids(sales, "ip_ticket_id"), ...ids(discounts, "ip_ticket_id")])];
  const ipTickets = await inRows(admin, "ip_tickets", "id,ticket_number,patient_id,is_emergency,patient_linked_at,patient_linked_by,doctor_id,source_visit_id,room,bed,admission_reason,status,admission_at,discharge_at,final_diagnosis,hospital_course,treatment_summary,discharge_medicines,discharge_advice,follow_up,discount_paise,created_by,created_at,updated_at", "id", ticketIds);
  ...
  const patientIds = [...new Set([...ids(visits, "patient_id"), ...ids(sales, "patient_id"), ...ids(reports, "patient_id"), ...ids(ipTickets, "patient_id"), ...ids(discounts, "patient_id")])];

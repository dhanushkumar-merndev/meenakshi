...
async function collectExport(month: string, includeDocuments: boolean) {
  ...
  const [visitsR, salesR, ipChargesR, ipPaymentsR, counterCollectionsR, admissionsR, reportsR, auditR, batchesR, settingsR] = await Promise.all([
    ...
    admin.from("ip_inventory_requests").select("id,ip_ticket_id,settlement,counter_collected_paise,counter_payment_mode,counter_reference,counter_collected_at,counter_collected_by,fulfilled_at,fulfilled_by,created_at").eq("settlement", "pharmacy_counter").gte("counter_collected_at", from).lt("counter_collected_at", to),
  ...
  for (const result of [visitsR, salesR, ipChargesR, ipPaymentsR, counterCollectionsR, admissionsR, reportsR, auditR, batchesR, settingsR]) if (result.error) throw result.error;
  ...
  const ticketIds = [...new Set([...ids((admissionsR.data ?? []) as Row[], "id"), ...ids(ipCharges, "ip_ticket_id"), ...ids(ipPayments, "ip_ticket_id"), ...ids(counterCollections, "ip_ticket_id"), ...ids(sales, "ip_ticket_id")])];

...
export default async function SalesPage({ searchParams }: { searchParams: Promise<{ page?: string; q?: string }> }) {
  ...
  return <div><PageHeader title="Pharmacy Sales" description="Prescription sales and IP items supplied; IP items remain payable on the patient's IP ticket" /><DebouncedSearchInput className="mb-4 max-w-md" initialValue={q} placeholder="Search patient name or phone" ariaLabel="Search pharmacy sales" /><Card><CardContent className="p-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Date/Time</TableHead><TableHead>Patient</TableHead><TableHead>Source</TableHead><TableHead>Items</TableHead><TableHead>Total</TableHead><TableHead>Dispensed By</TableHead><TableHead className="text-right">Record</TableHead></TableRow></TableHeader><TableBody>{rows.length ? rows.map((sale) => {
    const isIpItem = sale.source === "ip_items";
    return <TableRow key={`${sale.source}-${sale.id}`}><TableCell>{formatHospitalDate(sale.created_at, true)}</TableCell><TableCell><span className="font-medium">{sale.patient_name ?? "—"}</span><span className="block text-xs text-muted-foreground">{sale.patient_phone ?? ""}</span></TableCell><TableCell>{isIpItem ? <Badge variant="secondary">IP items</Badge> : <span className="uppercase">{sale.source}</span>}</TableCell><TableCell>{sale.item_count}</TableCell><TableCell>{formatInr(sale.total_paise)}</TableCell><TableCell>{sale.dispensed_by ?? "—"}</TableCell><TableCell className="text-right">{isIpItem ? <Badge variant="outline">Billed to IP</Badge> : <Button size="sm" variant="outline" render={<Link href={`/print/receipt/${sale.id}`} target="_blank" />}><Printer /> Receipt</Button>}</TableCell></TableRow>;

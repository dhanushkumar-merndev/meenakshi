...
export default async function SettingsPage() {
  ...
  const [{ data }, { data: usage }] = await Promise.all([
    supabase.from("hospital_settings").select("hospital_name,tagline,address,phone,email,prescription_footer,token_footer,digital_prescription_text,print_fee_on_prescription").eq("id", true).maybeSingle(),
    supabase.rpc("storage_usage_summary"),
  ]);
  const settings = data ?? { hospital_name: "Meenakshi Hospital", tagline: null, address: null, phone: null, email: null, prescription_footer: null, token_footer: null, digital_prescription_text: null, print_fee_on_prescription: false };

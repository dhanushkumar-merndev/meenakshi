import { requirePermission } from "@/lib/auth/dal";
import { PrintButton } from "@/components/shared/print-button";
import { IpBillDocument } from "@/features/ip/bill-document";
import { getIpPrintData } from "@/features/ip/print-data";
import { getHospitalIdentity } from "@/lib/print/hospital-identity.server";
export default async function IpTicketPrintPage({params}:{params:Promise<{id:string}>}){await requirePermission("manageIp");const{id}=await params;const[ticket,identity]=await Promise.all([getIpPrintData(id),getHospitalIdentity()]);return <main className="mx-auto min-h-screen max-w-[210mm] bg-white py-4 text-black sm:py-8"><div data-print-hidden className="mb-4 flex justify-end"><PrintButton label="Print Running Bill"/></div><IpBillDocument ticket={ticket} identity={identity}/></main>}
